...
export async function GET() {
  try {
    ...
    const feed = await parser.parseURL("https://www.eurogamer.net/feed");
    clearTimeout(timeoutId);

    if (!feed || !feed.items || !Array.isArray(feed.items)) {
      console.warn("Eurogamer feed returned invalid data");
    ...
    const formattedItems = feed.items
      ...
      .map((item) => ({
        ...
        image:
          ...
          item.enclosure?.url ||
          "/default-news.jpg",
        author: item.creator || item.author || "Eurogamer",
    ...
    console.error("Eurogamer API error:", error);
