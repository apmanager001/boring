...
export async function GET() {
  try {
    ...
    const feed = await parser.parseURL("https://www.pcgamer.com/rss/news/");
    clearTimeout(timeoutId);

    if (!feed || !feed.items || !Array.isArray(feed.items)) {
      console.warn("PCGamer feed returned invalid data");
    ...
    const formattedItems = feed.items
      ...
      .map((item) => ({
        ...
        image:
          ...
          item.enclosure?.url ||
          "/default-news.jpg",
        author: item.creator || item.author || "PCGamer",
    ...
    console.error("PCGamer API error:", error);
